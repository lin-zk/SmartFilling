...
"""
from PyQt5.QtWidgets import QWidget, QTableWidget, QAbstractItemView, QMessageBox, QTableWidgetItem, \
...
class AdminWindow(QWidget):
    def __init__(self):
        ...
        self.database = Database('./database/user.db')
    ...
    def add_table(self):
        ...
        self.table.setHorizontalHeaderLabels(["Choice", "username", "password", 'created_time', "latest_time"])  # 设置首行
    ...
    def get_all_user(self):
        """获取所有的用户信息"""
        data = self.database.read_table()  # 从数据库中获取用户信息，用户信息以 username, password, created_time, latest_time 形式返回
        for user in data:
            self.add_row(user[0], user[1], user[2], user[3])

    def add_row(self, username, password, created_time, latest_time):
        """在表格上添加一行新的内容"""
        ...
        self.table.setItem(row, 4, QTableWidgetItem(str(latest_time)))
        # 设置复选框
        widget = QWidget()
        check = QCheckBox()
    ...
    def clear(self):
        ...
        shutil.rmtree(r'.\database')
        os.mkdir(r'.\database')#删除所有数据库
